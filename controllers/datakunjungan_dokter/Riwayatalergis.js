import RAs from "../../models/datakunjungan_dokter/RiwayatalergiModel.js";
...
export const createRA = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ msg: "Data Riwayat Alergi Berhasil Ditambahkan", riwayatalergi });
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
}
...
export const deleteRA = async(req, res) =>{
    try {
        ...
        if(req.role === "pasien"){
            ...
            if(req.pasienId !== riwayatalergi.pasienId) return res.status(403).json({msg: "Akses terlarang"});
            await RAs.destroy({
                where:{
                    [Op.and]:[{id: riwayatalergi.id}, {pasienId: req.pasienId}]
                }
