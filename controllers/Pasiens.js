...
export const getPasiens = async(req, res) => {
    try {
        let response;
        if(req.role === "admin" || req.role === "dokter" || req.role === "apoteker"){
            response = await Pasiens.findAll({
                attributes:['uuid','nobpjs','nama','statuspeserta','tgllahir','gender','ppkumum', 'nohp', 'norm', 'createdAt'],
                include:[{
                    model: Users,
...
export const createPasien = async(req, res) => {
    const {nobpjs, nama, statuspeserta, tgllahir, gender, ppkumum, nohp, norm ,role} = req.body;
    try {
...
export const updatePasien = async(req, res) => {
    try {
        ...
        if(!pasien) return res.status(404).json({msg: "Data not found!"});
        const {nobpjs, nama, statuspeserta, tgllahir, gender, ppkumum, nohp, norm} = req.body;
        if(req.role === "admin"){
            await Pasiens.update({nobpjs, nama, statuspeserta, tgllahir, gender, ppkumum, nohp, norm},{
                where:{
                    id: pasien.id
                }
            });
